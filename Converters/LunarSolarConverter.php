...
{
    /** @var array */
    private static array $lunar_month_days = [
    ...
    /** @var array */
    private static array $solar_1_1 = [
    ...
    {
        $days = LunarSolarConverter::$lunar_month_days[$lunar->year - LunarSolarConverter::$lunar_month_days[0]];
        $leap = LunarSolarConverter::getBitInt($days, 4, 13);
        $offset = 0;
        $looped = $leap;
        if (!$lunar->isLeap) {
            if ($lunar->month <= $leap || $leap == 0) {
                $looped = $lunar->month - 1;
            } else {
                $looped = $lunar->month;
            }
        }
        ...
        $solar11 = LunarSolarConverter::$solar_1_1[$lunar->year - LunarSolarConverter::$solar_1_1[0]];
